...
export default function Home() {
  ...
  const handleInspirationSubmit = (e: React.FormEvent) => {
    ...
    // router.push(`/search?type=inspiration&start=${inspirationForm.start}&end=${inspirationForm.end}`)
    console.log('Inspiration search:', inspirationForm)
  }

  const handleSpecificSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    // Here you would typically handle the form submission, e.g.:
    // router.push(`/search?type=specific&start=${specificForm.start}&end=${specificForm.end}&startDate=${specificForm.startDate}&endDate=${specificForm.endDate}`)
    console.log('Specific search:', specificForm)
  }
          ...
          <form onSubmit={handleInspirationSubmit} className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="Enter start location e.g. NYCA"
                required
              />
            </div>
            <div>
              ...
              <Input
                ...
                placeholder="Enter end location e.g. PARI"
                required
              />
            </div>
